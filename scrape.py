...
def scrape_tournament_history(url, tournament):
    ...
    for x in range(0, len(rows)):
        ...
        data = OrderedDict()
        data['date'] = columns[0].get_text().replace('\n', '').strip(),
        data['patch'] = columns[1].get_text().replace('\n', '').strip(),
...
def scrape_match(match):
    ...
    for player in range(0, 1):
        ...
        # player
        # position
        # position = (match_data['participants'][player]['participantId']-1) % 5
        position = player % 5
        if position == 0:
            data['position'] = "top"
        elif position == 1:
        ...
        # bans
        # game length
        data['game_length'] = match_data['gameDuration']
        # result
        if match_data['participants'][player]['teamId'] == 100:
            team = 0
        else:
        ...
        # kda
        if data['deaths'] == 0:
            data['kda'] = (data['kills'] + data['assists'])/1
        else:
        ...
        for frame in timeline_data['frames']:
            ...
            for event in frame['events']:
                if event['type'] == "CHAMPION_KILL":
                    ...
                    break
        data['first_blood_time'] = round(first_blood_time/float(1000))
        # kills per minute
        data['kpm'] = data['kills']/float(data['game_length']/float(60))
        # opponent kills per minute
        data['okpm'] = data['opp_kills']/float(data['game_length']/float(60))
        # combined kills per minute
        data['ckpm'] = (data['kills'] + data['opp_kills'])/float(data['game_length']/float(60))
        ...
        if match_data['participants'][player]['teamId'] == 100:
            ...
            team = 1
        data['first_tower'] = match_data['teams'][team]['firstTower']
        ...
        for frame in timeline_data['frames']:
            ...
            for event in frame['events']:
                if event['type'] == "BUILDING_KILL":
                    ...
                    break
        data['first_tower_time'] = round(first_tower_time/float(1000))
            ...
            for event in frame['events']:
                if event['type'] == "BUILDING_KILL" and event['buildingType'] == "TOWER_BUILDING":
                    if event['teamId'] == 200:
                        ...
                        red_tower_kills += 1
        if match_data['participants'][player]['teamId'] == 100:
            data['first_to_three_towers'] = int(bool(blue_tower_kills))
        else:
            data['first_to_three_towers'] = int(bool(red_tower_kills))
        ...
        data['opponent_tower_kills'] = match_data['teams'][1-team]['towerKills']
        # first dragon
        if match_data['participants'][player]['teamId'] == 100:
            team = 0
        else:
            team = 1
        data['first_dragon'] = match_data['teams'][team]['firstDragon']
        # first dragon time
        first_dragon_time = 0
        for frame in timeline_data['frames']:
            ...
            for event in frame['events']:
                if event['type'] == "ELITE_MONSTER_KILL" and event['monsterType'] == "DRAGON":
                    ...
                    break
        data['first_dragon_time'] = round(first_dragon_time/float(1000))
        ...
        data['opponent_dragons'] = match_data['teams'][1-team]['dragonKills']
        # team & opponent elementals
        team_elementals = 0
        opponent_elementals = 0
        for frame in timeline_data['frames']:
            for event in frame['events']:
                if event['type'] == "ELITE_MONSTER_KILL" and 'monsterSubType' in event:
                    if event['monsterSubType'] != "ELDER_DRAGON":
                        if match_data['participants'][event['killerId']-1]['teamId'] == match_data['participants'][player]['teamId']:
                            ...
                            opponent_elementals += 1
        data['team_elementals'] = team_elementals
        data['opponent_elementals'] = opponent_elementals
        # team dragon types
        fire_dragons = 0
        water_dragons = 0
        earth_dragons = 0
        ...
        data['opponent_elders'] = opponent_elders
        # first baron
        if match_data['participants'][player]['teamId'] == 100:
            team = 0
        else:
            team = 1
        data['first_baron'] = match_data['teams'][team]['firstBaron']
        # first baron time
        first_baron_time = 0
        for frame in timeline_data['frames']:
            ...
            for event in frame['events']:
                if event['type'] == "ELITE_MONSTER_KILL" and event['monsterType'] == "BARON_NASHOR":
                    ...
                    break
        data['first_baron_time'] = round(first_baron_time/float(1000))
        ...
        data['opponent_barons'] = match_data['teams'][1-team]['baronKills']
        # herald
        if match_data['participants'][player]['teamId'] == 100:
            team = 0
        else:
            team = 1
        data['herald'] = match_data['teams'][team]['firstRiftHerald']
        # herald time
        herald_time = 0
        for frame in timeline_data['frames']:
            if herald_time != 0:
            ...
            for event in frame['events']:
                if event['type'] == "ELITE_MONSTER_KILL" and event['monsterType'] == "RIFTHERALD":
                    ...
                    break
        data['herald_time'] = round(herald_time/float(1000))
        ...
        data['damage_per_minute'] = match_data['participants'][player]['stats']['totalDamageDealtToChampions']/float(data['game_length']/float(60))
        ...
        data['gold_earned_per_minute'] = data['gold_earned']/float(data['game_length']/float(60))
        ...
        data['creep_score_per_minute'] = data['creep_score']/float(data['game_length']/float(60))
        ...
        data['monster_kills_opponent_jungle'] = match_data['participants'][player]['stats']['neutralMinionsKilledEnemyJungle']
        ...
        data['wards_placed_per_minute'] = data['wards_placed']/float(data['game_length']/float(60))
        ...
        data['wards_cleared_per_minute'] = data['wards_cleared']/float(data['game_length']/float(60))
        ...
        data['control_wards_placed_per_minute'] = control_wards_placed/float(data['game_length']/float(60))
        # opponent visible wards cleared %, opponent invisible wards cleared %
        opponent_visible_wards_placed = 0
        visible_wards_cleared = 0
        opponent_invisible_wards_placed = 0
            ...
            for event in frame['events']:
                ...
                if event['type'] == "WARD_PLACED" and event['wardType'] != "CONTROL_WARD" and event['wardType'] != "BLUE_TRINKET":
                    if player < 5 and ((event['creatorId']-1) >= 5):
                        opponent_visible_wards_placed += 1
                ...
                if event['type'] == "WARD_PLACED" and (event['wardType'] == "CONTROL_WARD" or event['wardType'] == "BLUE_TRINKET"):
                    if player < 5 and ((event['creatorId']-1) >= 5):
                        opponent_invisible_wards_placed += 1
                    elif player >= 5 and ((event['creatorId']-1) < 5):
                        opponent_invisible_wards_placed += 1
                if event['type'] == "WARD_KILL" and (event['wardType'] == "CONTROL_WARD" or event['wardType'] == "BLUE_TRINKET"):
                    if player < 5 and ((event['killerId']-1) >= 5):
                        ...
                        invisible_wards_cleared += 1
        data['visible_wards_cleared_percent'] = visible_wards_cleared/float(opponent_visible_wards_placed)
        data['invisible_wards_cleared_percent'] = invisible_wards_cleared/float(opponent_invisible_wards_placed)
